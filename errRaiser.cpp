...
void RaiseErr(ERRORTYPE _type, const std::string& message)
{
    ...
    if (_type == UNDEFINED_FUNC) std::cerr << "function '" << message << "' is not defined. " << std::endl;
    else if (_type == UNDEFINED_VAR) std::cerr << "variable '" << message << "' is not defined. " << std::endl; 
